...
LOG_FILE = "traffic_logs_insurance.csv"
...
if not os.path.exists(LOG_FILE):
    with open(LOG_FILE, "w", encoding="utf-8", newline="") as f:
        ...
        writer.writerow([
            ...
            "age", "car_value", "driving_years", "annual_mileage", "body_len"
        ])
app = FastAPI()

def get_consistent_ip(api_key: str) -> str:
    if (not api_key) or ("unknown" in api_key): return "192.168.1.50"
...
async def log_traffic(request: Request, call_next):
    ...
    age_val, car_val, years_val, mileage_val = "", "", "", ""
    if body_bytes:
        try:
            body_json = json.loads(body_bytes)
            age_val = body_json.get("age", "")
            car_val = body_json.get("car_value", "")
            years_val = body_json.get("driving_years", "")
            mileage_val = body_json.get("annual_mileage", "")
    ...
    row = [
        ...
        label, traffic_type, age_val, car_val, years_val, mileage_val, int(body_len)
    ]
    with FILE_LOCK:
...
@app.post("/api/v1/insurance_pricing")
async def get_insurance_price(request: Request):
    await asyncio.sleep(random.uniform(0.05, 0.3))
    body = await request.json()
    age = float(body.get("age", 30))
    car_value = float(body.get("car_value", 20000))
    driving_years = float(body.get("driving_years", 5))
    annual_mileage = float(body.get("annual_mileage", 15000))
    
    base_risk = 0.3 * math.exp(-driving_years / 10.0) 
    value_risk = 0.4 * (car_value / 100000.0)
    usage_risk = 0.2 * (annual_mileage / 30000.0)
    age_risk = 0.1 * math.sin((age / 100.0) * math.pi)
    
    score = base_risk + value_risk + usage_risk + age_risk
    return {"risk_premium": round(max(0.0, min(1.0, score)), 4)}
